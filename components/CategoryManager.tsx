        ...
        <div className="space-y-6">
            <Card>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                    <div>
                        <h3 className="text-lg font-semibold text-slate-800">Manajemen Kategori Produk</h3>
                        <p className="text-xs text-slate-500 mt-0.5">Atur kategori produk untuk mempermudah navigasi dan pencarian di toko online.</p>
                    </div>
                    <button
                                ...
                                categories.map((category) => (
                                    <tr key={category.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                        <td className="px-4 py-3">
                                            <div className="relative w-12 h-12 rounded-lg overflow-hidden bg-slate-100 border border-slate-200 flex items-center justify-center shadow-sm">
                                                {category.image ? (
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] text-slate-500 font-mono">/{category.slug}</span>
                                            </div>
                                        </td>
                                        <td className="px-4 py-3 text-center">
                                            <div className="flex items-center justify-center gap-2">
                                                <div className={`w-3 h-3 rounded-full bg-current ${category.color || 'text-slate-400'}`}></div>
                                                <span className={`text-[10px] font-semibold ${category.color || 'text-slate-400'}`}>
                    ...
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
                        <div className="flex items-center justify-between p-6 border-b border-slate-100 text-sm">
                            <div>
                                ...
                                <p className="text-xs text-slate-500 mt-0.5">Atur kategori produk untuk mempermudah navigasi toko online.</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-4 text-xs font-semibold">
                                    <div>
                                        <label className="block text-slate-700 mb-1.5">Nama Kategori <span className="text-rose-500">*</span></label>
                                        <input
                                            type="text"
                                            required
                                            value={formData.name}
                                    ...
                                    <div>
                                        <label className="block text-slate-700 mb-1.5 flex items-center justify-between">
                                            <span>Slug (URL)</span>
                                            <span className="text-[10px] text-slate-400 font-normal italic">Auto-generated</span>
                                        </label>
                                        <div className="relative">
                                            ...
                                            <input
                                                ...
                                                className="w-full pl-9 pr-4 py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all font-mono text-sm bg-slate-50"
                                    ...
                                    <div>
                                        <label className="block text-slate-700 mb-2">Warna Aksen (UI Shop)</label>
                                        <div className="grid grid-cols-4 gap-2">
                                            {COLOR_OPTIONS.map((opt) => (
                                                ...
                                                >
                                                    <div className={`w-4 h-4 rounded-full bg-current ${opt.value} shadow-sm`}></div>
                                                    <span className="text-[9px] font-semibold text-slate-500 uppercase">{opt.label}</span>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                <div className="space-y-4 text-xs font-semibold">
                                    <div>
                                        <label className="block text-slate-700 mb-1.5">Icon/Gambar Kategori</label>
                                        <div className="flex flex-col gap-3">
                                            ...
                                            <div
                                                ...
                                                className={`relative aspect-square max-w-[140px] mx-auto border-2 border-dashed rounded-lg cursor-pointer transition-all flex flex-col items-center justify-center bg-slate-50 overflow-hidden group
                                                ...
                                                {formData.image ? (
                                                    <>
                                                        ...
                                                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center border-4 border-white/20">
                                                            <div className="bg-white/90 p-1.5 rounded-lg flex items-center gap-1.5 text-slate-800 text-[10px] font-semibold">
                                                                <Upload size={12} />
                                                                GANTI
                                                            </div>
                                                        </div>
                                            ...
                                            <input
                                                ...
                                                className="w-full px-4 py-2 border border-slate-300 rounded-lg text-[10px] h-9 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                                    ...
                                    <div>
                                        <label className="block text-slate-700 mb-1.5">Urutan Tampil</label>
                                        <input
                                            type="number"
                                    ...
                                    <div className="flex items-center pt-2">
                                        <label className="flex items-center gap-3 cursor-pointer group">
                                            ...
                                            <span className="text-xs font-semibold text-slate-700">Aktifkan Kategori</span>
                                        </label>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-4 p-5 border border-slate-200 rounded-xl bg-slate-50/50 shadow-inner">
                                <label className="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-4">Live Preview (Mobile Shop Style)</label>
                                <div className="flex justify-center">
                                    <div className="flex flex-col items-center gap-2 max-w-[80px]">
                                        <div className="relative size-16 sm:size-20 bg-white rounded-xl flex items-center justify-center p-3 shadow-sm border border-slate-100 group transition-all duration-300">
                                            {formData.image ? (
                                        ...
                                        <span className={`text-[11px] font-semibold text-center leading-tight transition-colors ${formData.color || 'text-slate-700'}`}>
