...
export default function CheckoutPage() {
    ...
    const { items, subtotal, totalSavings, clearCart, couponCode, couponDiscount, applyCoupon: applyPublicCoupon, removeCoupon: removePublicCoupon } = useCart();
    ...
    // Voucher state (Personal Vouchers)
    const [voucherCode, setVoucherCode] = useState('');
    const [appliedVoucher, setAppliedVoucher] = useState<any>(null);
    const [isValidatingVoucher, setIsValidatingVoucher] = useState(false);
    const [voucherError, setVoucherError] = useState('');

    // Public Coupon state (Centralized via useCart)
    const [publicPromoCodeInput, setPublicPromoCodeInput] = useState('');
    const [isApplyingPublicCoupon, setIsApplyingPublicCoupon] = useState(false);
    const [publicCouponError, setPublicCouponError] = useState('');

    // Total calculation: sum up all discounts
    const voucherDiscount = appliedVoucher ? Number(appliedVoucher.value) : 0;
    const totalDiscount = voucherDiscount + couponDiscount;
    const total = Math.max(0, subtotal + shippingData.fee + serviceFee - totalDiscount);
    ...
    const handleApplyPublicCoupon = async () => {
        if (!publicPromoCodeInput.trim()) return;
        setIsApplyingPublicCoupon(true);
        setPublicCouponError('');
        const result = await applyPublicCoupon(publicPromoCodeInput);
        if (!result.success) {
        ...
        <>
            ...
            <div className="p-4 pb-24 flex flex-col gap-6 max-w-md mx-auto">
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                    ...
                    {/* Show applied coupon */}
                    {couponCode && (
                        <div className="bg-orange-50 border border-orange-100 p-3 rounded-xl flex items-center justify-between mb-3">
                            <div>
                                <p className="text-[10px] font-black text-orange-600 tracking-wider">PROMO TERAPLIKASI</p>
                                <p className="text-sm font-bold text-orange-800 uppercase">{couponCode}</p>
                                <p className="text-[10px] text-orange-700">Potongan Rp {couponDiscount.toLocaleString('id-ID')}</p>
                            </div>
                            <button
                                onClick={() => { removePublicCoupon(); setPublicPromoCodeInput(''); }}
                    ...
                    {appliedVoucher && (
                        <div className="bg-purple-50 border border-purple-100 p-3 rounded-xl flex items-center justify-between mb-3">
                            <div>
                                <p className="text-[10px] font-black text-purple-600 tracking-wider">VOUCER TERAPLIKASI</p>
                                <p className="text-sm font-bold text-purple-800">{appliedVoucher.code}</p>
                                <p className="text-[10px] text-purple-700">Potongan Rp {Number(appliedVoucher.value).toLocaleString('id-ID')}</p>
                            </div>
                            <button
                    ...
                    {/* Single input for promo/voucher */}
                    {!couponCode && !appliedVoucher && (
                        <div className="space-y-3">
                            <div className="flex gap-2">
                                <input
                                    type="text"
                                    value={publicPromoCodeInput || voucherCode}
                                    onChange={(e) => {
                                        const val = e.target.value.toUpperCase();
                                        setPublicPromoCodeInput(val);
                                        setVoucherCode(val);
                                    }}
                                ...
                                <button
                                    onClick={async () => {
                                        const code = publicPromoCodeInput || voucherCode;
                                        if (!code.trim()) return;

                                        // Try public coupon first
                                        setIsApplyingPublicCoupon(true);
                                        setPublicCouponError('');
                                        setVoucherError('');

                                        const couponResult = await applyPublicCoupon(code);
                                        ...
                                        // If authenticated, try as personal voucher
                                        if (isAuthenticated) {
                                            setIsValidatingVoucher(true);
                                            try {
                                                const res = await fetch('/api/shop/vouchers/validate', {
                                                ...
                                                // Ignore voucher error, show coupon error
                                            }
                                            setIsValidatingVoucher(false);
                                        }

                                        // Show error if both failed
                                        setPublicCouponError(couponResult.message);
                                        setIsApplyingPublicCoupon(false);
                                    }}
                                    disabled={isApplyingPublicCoupon || isValidatingVoucher || (!publicPromoCodeInput && !voucherCode)}
                                    className="px-6 py-3 bg-stone-900 text-white rounded-xl text-sm font-bold disabled:opacity-50"
                                >
                                    {(isApplyingPublicCoupon || isValidatingVoucher) ? <Loader2 size={16} className="animate-spin" /> : 'Pakai'}
                                </button>
                            </div>
                            {(publicCouponError || voucherError) && (
                                <p className="text-xs text-rose-500 px-1 font-medium">{publicCouponError || voucherError}</p>
                            )}
                ...
                <div className="bg-white p-4 rounded-xl border border-gray-200">
                    ...
                    <div className="space-y-2 text-sm">
                        ...
                        {couponDiscount > 0 && (
                            <div className="flex justify-between">
                                <span className="text-orange-600 font-medium italic">Kupon Promo ({couponCode})</span>
                                <span className="font-medium text-orange-600">- Rp {couponDiscount.toLocaleString('id-ID')}</span>
                            </div>
                        )}
                        {totalDiscount > 0 && (
                            <div className="flex justify-between">
                                <span className="text-emerald-600 font-medium italic">Total Potongan</span>
                                <span className="font-medium text-emerald-600">- Rp {totalDiscount.toLocaleString('id-ID')}</span>
                            </div>
                        )}
